...
puzzle = """[.###.#..] (0,2,3,4,6,7) (1,2,4,5,6) (1,3,4,5,7) (0,2,4) (4,7) (0,1,4,6) (1,2,3,4,5,7) (2,4,7) {29,44,42,26,66,27,30,36}
...
for line in lines:
    parts = line.split(" ")
    indicators.append([1 if indicator == "#" else 0 for indicator in parts[0][1:-1]])
    buttons.append([tuple(map(int, button[1:-1].split(","))) for button in parts[1:-1]])
    joltage.append(list(map(int, parts[-1].strip("{}").split(","))))

# print(indicators[0], buttons[0], joltage[0])

def button2binary(button, length):
    binary = [0]*length
    for pos in button:
        binary[pos] = 1
    return binary

# def brute_force(indicator, buttons):
#     # Generate all possible combinations of button presses
#     n_buttons = len(buttons)
#     min_presses = float('inf')
#     best_combination = None
#     for i in range(1 << n_buttons):
#         combined_indicator = [0]*len(indicator)
#         presses = 0
#         for j in range(n_buttons):
#             if (i >> j) & 1:
#                 presses += 1
#                 button_binary = button2binary(buttons[j], len(indicator))
#                 combined_indicator = [a ^ b for a, b in zip(combined_indicator, button_binary)]
#         if combined_indicator == indicator:
#             if presses < min_presses:
#                 min_presses = presses
#                 best_combination = [buttons[j] for j in range(n_buttons) if (i >> j) & 1]
#     return min_presses, best_combination

# print(brute_force(indicators[17], buttons[17]))
# button_sequence = buttons[17][1:]
# print(button_sequence)
# indicator = reduce(lambda x, y: [a ^ b for a, b in zip(x, y)], (button2binary(button, len(indicators[17])) for button in button_sequence), [0]*len(indicators[17]))
# print(indicator)

# def solve_mlp(indicators, buttons, joltage):
#     n_buttons = len(buttons)
#     n_indicators = len(indicators)
#     A = np.array([button2binary(button, len(indicators)) for button in buttons])
#     A = np.c_[A.T, -2*np.eye(n_indicators, dtype=int)]  # add parity constraint

#     c = np.zeros(n_indicators)
#     c[:n_buttons] = 1  # minimize number of pressed buttons

#     lb = np.zeros(n_buttons + n_indicators)
#     ub = np.ones(n_buttons).tolist() + [1000]*n_indicators  # big enough
#     bounds = Bounds(lb, ub)

#     integrality = np.ones(n_buttons + n_indicators, dtype=int)
#     integrality[:n_buttons] = 1  # buttons binary
#     integrality[n_buttons:] = 1  # parity integers (unbounded)

#     lc = LinearConstraint(A, indicators, indicators)

#     # Cost vector
#     c = np.zeros(A.shape[1])
#     c[:n_buttons] = 1  # minimize number of buttons pressed

#     res = milp(c, constraints=[lc], integrality=np.ones(A.shape[1]), bounds=bounds, options=dict(mip_rel_gap=0, presolve=False, disp=False))

#     return np.ceil(res.fun)




def solve_mlp_joltage(buttons, joltage, big_M=1000):
    """
    Solve MILP for number of button presses.

    Parameters
    ----------
    buttons : list
        Each button converted to a binary effect vector via button2binary(button, n_indicators)
    joltage : array-like of shape (n_indicators,)
        Target sum for each indicator
    big_M : int
        Upper bound for number of times a button can be pressed (must be >= any feasible solution)

    Returns
    -------
    total_presses : float
        Minimum total number of button presses to achieve joltage
    """

    n_buttons = len(buttons)
    n_indicators = len(joltage)

    # --- Build button effect matrix ---
    # Each column = one button, each row = one indicator
    A = np.array([button2binary(btn, n_indicators) for btn in buttons], dtype=float).T

    # --- Bounds ---
    lb = np.zeros(n_buttons, dtype=float)
    ub = big_M * np.ones(n_buttons, dtype=float)
    bounds = Bounds(lb, ub)

    # --- Integrality ---
    integrality = np.ones(n_buttons, dtype=int)  # all variables integer

    # --- Constraint: A x = joltage ---
    joltage_constraint = LinearConstraint(A, joltage, joltage)

    # --- Cost vector: minimize total button presses ---
    ...
    res = milp(
        ...
        options=dict(mip_rel_gap=0, presolve=True, disp=False)
    )

    if not res.success:
        raise ValueError(f"MILP could not find a solution: {res.message}")

    return np.ceil(res.fun)  # total button presses

# print(sum(solve_mlp(indicators[i], buttons[i]) for i in range(len(indicators))))
# print([solve_mlp(indicators[i], buttons[i], joltage[i]) for i in range(len(indicators))])
print(sum(solve_mlp_joltage(buttons[i], joltage[i]) for i in range(len(indicators))))
# print([solve_mlp(indicators[i], buttons[i]) for i in range(len(indicators))])